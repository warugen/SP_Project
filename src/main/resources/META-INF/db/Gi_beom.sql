...
--쿠폰 갯수 불러오기--
SELECT COUNT(*) FROM COUPON_HOLD WHERE CID='aaaa';
--주소록 불러오기--
...
SELECT COUNT(*)	FROM PRODUCT P  WHERE PCODE IN 
                    (SELECT DISTINCT PO.PCODE FROM PRODUCT_OPTION PO,PRODUCT PP WHERE PO.PCODE=PP.PCODE 
                        AND TYPECODE LIKE '%'||''||'%'  AND PTITLE LIKE '%'||'쿠첸'||'%');
...
SELECT * FROM (SELECT ROWNUM RN, A.* FROM(SELECT p.*,
			(SELECT MIN(POPRICE) FROM PRODUCT_OPTION WHERE PCODE = P.PCODE) POPRICE 
				FROM PRODUCT P  WHERE PCODE IN 
                    (SELECT DISTINCT PO.PCODE FROM PRODUCT_OPTION PO,PRODUCT PP WHERE PO.PCODE=PP.PCODE AND TYPECODE LIKE '%'||''||'%'
                        AND PTITLE LIKE '%'||'쿠첸'||'%')
